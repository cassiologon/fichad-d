/**
 * Lista de vantagens do 3DT Alpha com seus custos em pontos, resumos e descrições completas
 */

/**
 * Função para gerar resumo automático baseado na descrição
 * Extrai informações chave como bônus, custos em PMs e efeitos principais
 */
function generateSummary(description) {
  ...
  // Extrair bônus em características (ex: +1 H, +2 F, PdF+1)
  const bonusPatterns = [
    /(\+?\d+)\s*(pontos?\s*)?(à|na|em)?\s*(Habilidade|H|Força|F|Resistência|R|Armadura|A|Poder de Fogo|PdF)/gi,
    /(H|F|R|A|PdF)\s*\+?\s*(\d+)/gi,
    /(Habilidade|Força|Resistência|Armadura|Poder de Fogo)\s*\+?\s*(\d+)/gi
  ]
  
  // Extrair custos em PMs
  const pmPatterns = [
    /(?:gasta|gastar|gastando|custa|custar|consome|consumir)\s*(\d+)\s*(?:Pontos?\s*de\s*Magia|PMs?)/gi,
    /(\d+)\s*(?:Pontos?\s*de\s*Magia|PMs?)/gi
  ]
  
  // Extrair efeitos principais
  const effects = []
  ...
  bonusPatterns.forEach(pattern => {
    ...
    if (matches) {
      matches.forEach(match => {
        if (!summaryParts.includes(match.trim())) {
          summaryParts.push(match.trim())
        }
      })
    }
  })
  
  // Procurar por custos em PMs
  pmPatterns.forEach(pattern => {
    const matches = description.match(pattern)
    if (matches) {
      matches.forEach(match => {
        const pmMatch = match.match(/(\d+)\s*(?:Pontos?\s*de\s*Magia|PMs?)/i)
        if (pmMatch && !summaryParts.some(p => p.includes(pmMatch[1] + ' PM'))) {
          summaryParts.push(`custa ${pmMatch[1]} PM`)
        }
      })
    }
  })
  
  // Se não encontrou nada específico, pegar primeira frase
...
export const advantages = [
  ...
  },
  {
    name: 'Energia Extra',
    cost: 1,
    summary: 'Recupera todos os PVs, custa 2 PMs, só quando Perto da Morte',
    description: 'Você consegue invocar forças interiores para se recuperar espantosamente, recarregando seus Pontos de Vida. Você pode gastar 2 PMs para recuperar todos os seus PVs. Usar a Energia Extra leva um turno inteiro, e você não pode fazer qualquer outra coisa enquanto se concentra. Existem dois níveis para esta vantagem: por 1 ponto você só pode usar a Energia Extra quando estiver Perto da Morte (veja em "Pontos de Vida"). Mas por 2 pontos a Energia Extra pode ser usada a qualquer momento. Energia Extra é parecida com magia de cura, mas serve apenas para curar você mesmo, e não funciona contra qualquer outro tipo de mal (doenças, veneno...).'
  },
  {
    name: 'Energia Extra (Qualquer Momento)',
    cost: 2,
    summary: 'Recupera todos os PVs, custa 2 PMs, a qualquer momento',
    description: 'Você consegue invocar forças interiores para se recuperar espantosamente, recarregando seus Pontos de Vida. Você pode gastar 2 PMs para recuperar todos os seus PVs. Usar a Energia Extra leva um turno inteiro, e você não pode fazer qualquer outra coisa enquanto se concentra. Por 2 pontos a Energia Extra pode ser usada a qualquer momento. Energia Extra é parecida com magia de cura, mas serve apenas para curar você mesmo, e não funciona contra qualquer outro tipo de mal (doenças, veneno...).'
  },
  {
    name: 'Energia Vital',
    cost: 2,
    summary: 'Pode usar PVs no lugar de PMs (2 PVs = 1 PM)',
    description: 'Você pode usar sua própria energia vital para alimentar magias e poderes especiais, ainda que fazer isso provoque dano. Em vez de Pontos de Magia, você pode usar Pontos de Vida para ativar vantagens, lançar magias ou fazer qualquer outra coisa que exige o gasto de PMs. 2 PVs valem 1 PM. Você ainda recupera PVs e PMs normalmente, e também pode usar seus Pontos de Magia normais.'
  },
  {
    name: 'Familiar',
    cost: 1,
    summary: 'Animal mágico que compartilha habilidades',
    description: 'Você tem um pequeno animal mágico que compartilha suas habilidades. Um Familiar une-se totalmente a seu dono, como um Parceiro. Eles passam a ser como uma só criatura, combinando suas características mais altas (provavelmente as do próprio mago) e vantagens. O conjurador e o Familiar também possuem uma Ligação Natural (veja adiante). Exemplo: um mago com F1, H4, R2, A3, PdF1 e Arena une-se a um corvo familiar com F0, H1, R1, A0, PdF0 e Voo. Agora a dupla terá F1, H4, R2, A3, PdF1, Arena e Voo (o mago também será capaz de voar). Estes são os animais que podem se tornar familiares: Camaleão: F0, H0, R1, A0, PdF0, Invisibilidade. Corvo (ou qualquer pássaro): F0, H1, R0, A0, PdF0, Voo. Gato: F0, H2, R0, A0, PdF0, Sentidos Especiais (Audição, Faro, Visão Aguçadas). Cão/Lobo: F1, H1, R1, A0, PdF0, Sentidos Especiais (Audição, Faro Aguçados). Macaco: F0, H1, R1, A0, PdF0, Arena (ermos). Sapo: F0, H0, R1, A0, PdF0, Arena (água). Serpente: F0, H1, R0, A0, PdF0, Paralisia. Apenas usuários de magia (Branca, Elemental ou Negra) podem receber os benefícios de um familiar. Nenhum mago pode ter mais de um familiar ao mesmo tempo. Nota: o mestre pode proibir a aquisição de um Familiar para um personagem recém-criado. Em vez disso, um Familiar custará 10 Pontos de Experiência, devendo ser conquistado em campanha. Se um Familiar morre, seu dono deve gastar 5 PEs para adquirir outro de mesmo tipo, ou 10 PEs para um Familiar de outro tipo.'
  },
  {
  ...
  },
  {
    ...
    cost: 2,
    summary: 'Conjurador de magia dos elementos (terra, água, fogo, ar, espírito)',
  ...
  },
  {
    name: 'Magia Negra',
    cost: 2,
    summary: 'Conjurador de magia negra, necromancia e demonologia',
  ...
  },
  {
    ...
    cost: 1,
    summary: 'Vítima sofre –1 em teste de Resistência',
    description: 'É mais difícil resistir às suas magias. O redutor sofrido pela vítima depende do custo da vantagem: 1 ponto: a vítima sofre –1 em seu teste de Resistência. 2 pontos: a vítima sofre –2 em seu teste de Resistência. 3 pontos: a vítima sofre –3 em seu teste de Resistência.'
  },
  {
    name: 'Magia Irresistível (+2)',
    cost: 2,
    summary: 'Vítima sofre –2 em teste de Resistência',
    description: 'É mais difícil resistir às suas magias. O redutor sofrido pela vítima depende do custo da vantagem: 2 pontos: a vítima sofre –2 em seu teste de Resistência.'
  },
  {
    name: 'Magia Irresistível (+3)',
    cost: 3,
    summary: 'Vítima sofre –3 em teste de Resistência',
    description: 'É mais difícil resistir às suas magias. O redutor sofrido pela vítima depende do custo da vantagem: 3 pontos: a vítima sofre –3 em seu teste de Resistência.'
  },
  {
    name: 'Membros Elásticos',
    cost: 1,
    summary: 'Braços/pernas podem atacar à distância com Força',
  ...
  },
  {
    name: 'Teleporte',
    cost: 2,
    summary: 'Desaparece e reaparece, +2 em esquivas, custa 1 PM',
  ...
  },
  {
    name: 'Xamã',
    cost: 1,
    summary: 'Ligação com espíritos, pode atacar fantasmas',
